		...
		<div class="container">	
			...
			<div class="wrap-slick2">
				<div class="slick2">
							...
							<?php
					$prodcatid= $proData['productcatid'];

						$catquery = $pdo->query("SELECT products.* ,categories.catname from products INNER JOIN categories on products.productcatid = categories.catid WHERE catid = $prodcatid AND productid != $prostringid");
						if($catquery->rowCount() > 0 ){
							$proData = $catquery ->fetchAll(PDO::FETCH_ASSOC);
						}
						else{
							$catsquery = $pdo->query("SELECT products.* ,categories.catname from products INNER JOIN categories on products.productcatid = categories.catid WHERE productid != $prostringid");
							$proData = $catsquery ->fetchAll(PDO::FETCH_ASSOC);
						}
